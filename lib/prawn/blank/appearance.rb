...
module Prawn::Blank
  class Appearance
    ...
    def text_field(element)
      text_style = element.text_style # ||= Prawn::Blank::TextStyle.new(@document,"Ruboto",9,'000000')
      border_style = element.border_style ||= { :W => 1, :Type => :Border, :S => :S }

      if !element.width or element.width <= 0
        element.width = 100
      end
      if !element.height or element.height <= 0
        element.height = (text_style || @document.font_size) + 6 + 2 * border_style[:W]
      end

      width = element.width
      height = element.height
      style = element.style ||= { :BC => @document.send(:normalize_color,'ffffff'), :BG => @document.send(:normalize_color,'000000') }
      # Prawn::ColorStyle(@document,'ffffff','000000')
